...
- 同一网盘剧集只显示一个作品入口，进入后按季度显示对应 TMDB 季度海报和真实选集。
- 视频文件名只有集号时，可从“剧名 第2季”或“剧名/第二季”等文件夹结构识别剧名和季度；文件名包含明确季号时优先采用文件名。
- 手动确认一个网盘分集的 TMDB 结果后，同目录同剧分集会立即和后续自动继承；已有匹配和自定义剧名不会被覆盖。
