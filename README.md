...
- 迅雷网盘为测试能力，可登录个人账号并选择多个媒体目录；账号密码不保存，设备验证在受限应用内窗口中完成并自动继续登录。
- 迅雷仅支持只读扫描、TMDB 刮削和原画播放，不提供上传、移动、改名、删除、分享转存或离线下载。
- 网盘资源页顶部菜单和空状态提供迅雷网盘与 OpenList 入口。
- 支持在线播放夸克媒体，以及手动查看、选择和转存夸克分享内容。
- 夸克默认转存目录会自动纳入媒体扫描范围；旧配置会在下次扫描时自动补齐，无需再次转存文件。
- 夸克与百度原文件通过公共本机分段中转边下边播，支持预读、拖动和临时鉴权自动更新；Android 夸克播放会按缓存压力自适应提速，分段缓存会在退出或切集后清理。
...
- 平铺在媒体根目录中的同系列裸集号文件会按剧名和集数归并，不会误显示成同一电影的“多个版本”；4K、BD、10bit 等规格数字不会抢占真实集号。
- 支持 `作品名 2 - 07` 等短横线集号，并从 `S02` 等作品目录继承季度；重新识别后保留已手动匹配的 TMDB 资料和对应季度海报。
- 重新识别或作品目录改名后，即使当前作品键残留旧“未匹配”记录，也会按保存的旧根路径恢复同一路径上的手动 TMDB 资料，并保留作品海报和季度海报。
- 视频目录下名为“外挂字幕”的直接子目录会参与字幕关联，同名或同集 ASS、SSA、SRT、VTT 字幕可在播放器中选择。
...
OpenList、夸克、百度和迅雷网盘在本项目中仅作为用户自有媒体的数据入口，不是在线影视搜索服务。
...
- 没有 Key 或断网时，本地扫描和播放仍可用；TMDB 不可用也不会阻断本地媒体库。
- TMDB 刮削不会修改或删除原始视频、字幕。

### 播放器

- 基于 media-kit 与 mpv，支持常见本地和远程媒体格式。
- 支持 Windows 硬件解码自动回退、Android MediaCodec、低内存模式和 TrueHD 兼容音轨回退。
...
- 查看并切换内嵌音轨和字幕轨，Android 支持通过 GPU 合成 PGS 图形字幕。
...
项目同时支持 Windows 与 Android。本轮交付 Windows 1.0.7 正式版 EXE 安装程序和 Android 1.0.4 正式版 mobile APK/AAB；
...
6. Windows 和 Android 可在“设置 > 配置迁移”中使用密码导出或导入 `.kyyconfig`。支持 OpenList、夸克、百度和迅雷来源，写入失败会回滚，不会修改或删除视频、字幕、索引、缓存和播放历史。
