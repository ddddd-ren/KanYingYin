...
> 网盘挂载目前仍在完善中，尚未完全实现，正式版请优先使用本地媒体库功能。
...
从 `v2.0.11` 测试版切换到 `v1.0` 正式版时，由于安装包版本号降低，需要先卸载测试版再安装正式版。卸载不会删除用户的原始视频文件。
...
```powershell
D:\flutter\bin\dart.bat run msix:create --build-windows false --certificate-password <证书密码>
```
