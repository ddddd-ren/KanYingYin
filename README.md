...
- 左侧“网盘资源”可直接切换来源、浏览目录并点击视频在线播放。
- 网盘文件夹和媒体根目录中的独立视频支持 TMDB 自动刮削，可显示海报、中文标题、评分和简介。
- 已刮削电影和剧集使用沉浸式海报卡，并保留真实网盘名称；不会重命名远程文件或改变播放路径。
- 支持手动重新匹配与刮削当前目录；单项操作可先修改搜索词、类型和年份，再从完整候选信息中确认结果。
- 支持修改网盘资源在看影音中的显示剧名，并可恢复 TMDB 中文标题；不会重命名远程文件或改变播放路径。
