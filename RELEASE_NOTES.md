...
Windows EXE 安装器版本：2.1.206

Android 手机测试版：2.1.206 (20206)
...
标题：看影音 Android 2.1.206 测试版

- Android 手机仅同步版本配置与共享修复，本轮不交付 Android 安装包。
- 本次更新不会修改、删除、改名或移动本地及个人网盘中的原始视频。
...
- Windows 媒体库、分类和网盘资源页继续保留加载状态与播放入口。
- 本轮仅交付 Windows 测试版 EXE，不构建 Android TV 安装包。
- 本次更新不会修改、删除、改名或移动本地及个人网盘中的原始视频、字幕和海报缓存。

### Android 手机测试版更新内容

标题：看影音 Android 2.1.204 测试版

- Android 手机测试版版本与 Windows 测试版保持一致，本轮不单独交付 Android 安装包。
- 本次更新不会修改、删除、改名或移动本地及个人网盘中的原始视频、字幕和海报缓存。

## 1.0.13+10013

Windows EXE 安装器版本：1.0.13
...
- 全屏通道会等待原生过渡完成后再同步播放器状态，避免切换过程中重复触发或控制条状态错乱。
...
标题：看影音 Android 2.1.199 测试版

- Android 2.1.199 测试版延续上一测试版的本地媒体库、个人网盘、字幕、音轨、后台播放、画中画、MediaCodec 硬件解码和 Anime4K 能力。
- 本次更新不会修改、删除、改名或移动本地及个人网盘中的原始视频、字幕和海报缓存。
...
- 窗口进入和退出全屏改用单次原生定位，减少尺寸重排造成的跳变，切换过程更平滑。
- 修复版本更新后部分发布契约测试仍读取旧版本号的问题，当前版本会从真实构建声明交叉校验。
- 清理网盘资源页已停用的旧目录浏览兼容入口，不影响现有来源级海报墙、扫描、刮削和播放。
- 播放网盘视频时不再在画面顶部显示常态读取速度；底部控制栏网速、预缓冲和低速或失败提示继续保留。
- 本轮交付 Windows 测试版 EXE 和 Android 手机测试版 APK；不构建 AAB 或 Android TV 安装包。
...
标题：看影音 Android 2.1.198 测试版

- Android 2.1.198 测试版汇总近期手机版更新，继续支持本地媒体库、个人网盘、字幕、音轨、后台播放、画中画、MediaCodec 硬件解码和 Anime4K。
...
Windows EXE 安装器版本：1.0.12

Android 正式版：1.0.8 (10008)
...
本轮仅交付 Windows 测试版，不构建 Android 手机或 Android TV
...
- 修复 Windows 使用外部播放器时，系统播放器可能把临时播放列表误判为 HLS 并提示视频解析异常的问题。
- Android 手机本轮不打包；Android TV 继续暂停。
- 本次更新不会修改、删除、改名或移动本地及个人网盘中的原始视频、字幕和海报缓存。
...
Android 正式版仍为 1.0.6 (10006)，本轮不打包
...
- 首次响应较慢时会继续显示缓冲并等待；本地 Range 中转会尽快返回响应头，避免约 5 秒后直接打开失败。
