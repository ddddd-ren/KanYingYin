# 看影音更新弹窗文案

## 当前版本

- 应用版本：2.1.109
- 安装包版本：2.1.109.0
- 本轮交付：Windows 测试版 MSIX
- Android 应用版本：1.0.2
- Android versionCode：10002
- 日期：2026-08-04

## 弹窗标题

看影音 2.1.109 测试版

## Windows 弹窗正文

- 本轮只提供 Windows 2.1.109 测试版 MSIX，不构建或测试 Android 安装包。
- 本地媒体库新增筛选入口，按 TMDB 信息提供“动漫”“电影”“电视剧”等分类，并保留 TMDB 类型筛选。
- 每个本地作品菜单新增“管理标签”，可以自己添加、删除和保存标签；标签重启后仍然可用。
- 类型和自定义标签支持多选，匹配任一标签；自定义标签只筛选本地媒体库，不会混入网盘资源。
- 网盘刮削完成后类型标签会立即用于当前网盘筛选，索引重试也会保留标签。
- 本次更新不会修改或删除本地及网盘原始视频、字幕或其他文件。
- 本次更新不会修改或删除本地及网盘原始视频、字幕或其他文件。

## Android 弹窗正文

### 弹窗标题

看影音 Android 1.0.2 正式版

- Android 1.0.2 正式版综合 2.1.101 至 2.1.103 的移动端功能更新。
- 诊断日志页面支持导出完整脱敏诊断包，并通过系统分享面板发送或保存；网盘凭据、Token、请求头和远程媒体完整地址不会进入导出内容。
- 为只有 TrueHD/MLP 音轨的视频提供软件解码和立体声输出，同时继续保留 MediaCodec 视频硬件解码。
- 横屏播放器保持沉浸显示，旋转、恢复前台或重新获得焦点后会继续隐藏系统栏；普通页面的状态栏和底部手势区与界面同色。
- 夸克原画播放会根据缓存压力自适应提速，最多使用八路读取和七路后台预取，临时缓存上限为 192 MiB；重新连接后会先回到稳健档位，缓存恢复后再按实际压力提速。
- 百度及迅雷网盘读取策略保持不变。
- 本次更新不会修改或删除本地及网盘原始视频、字幕或其他文件。

## 按钮文字

知道了

## 维护要求

发布新版本时，本文件中的应用版本、安装包版本和弹窗正文必须与以下文件保持一致：

- `pubspec.yaml`
- `lib/core/app_version.dart`
- `lib/utils/version_history.dart`
- `RELEASE_NOTES.md`
