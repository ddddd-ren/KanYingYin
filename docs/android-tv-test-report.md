...
- 桌面 APK：`看影音-2.1.138-TV测试版.apk`
- APK SHA-256：`e0e20b504cbe762720e75733fb7049b4d3bbbce547c97228c1693bf68df85004`
...
用户在海信电视上反馈：焦点停留在侧栏外，搜索框吞掉方向键，第一次返回直接退出应用。

已加入自动化回归并修复：

- Android TV 搜索框的上下左右键先尝试离开输入框并进入方向焦点遍历，桌面和普通 Android 仍保留文本编辑行为。
- 搜索框有焦点时第一次返回只清除输入焦点。
- TV 根页面第一次返回显示退出确认；确认框内再次按返回才执行退出，也可用遥控器选择取消或退出。
