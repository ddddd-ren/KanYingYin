...
2.1.144 的自动化、Windows Release、Inno 安装器构建和 Android TV 包验证已经通过。当前 ADB 设备列表为空，未安装到用户的海信电视，因此交付结论保持“构建与包验证通过，海信实机验收未完成”。

## 质量门禁

- Dart 格式：`679` 个文件检查完成，`0` 个文件需要修改。
- Flutter 测试：`1793/1793` 通过。
...
- Release 主程序产品版本：`2.1.144`。
- 桌面 Inno 安装器：`C:\Users\asus\Desktop\看影音-2.1.144-测试版-安装程序.exe`。
- 安装器大小：`69814537` 字节。
- 安装器产品版本：`2.1.144`。
- 安装器 SHA-256：`05A774031AD5B7A80259E04E061D21FB23020DB5279782A7C31333894C31C028`。
...
- 桌面 APK：`C:\Users\asus\Desktop\看影音-2.1.144-TV测试版.apk`。
- APK 大小：`134647154` 字节。
- 源 APK 与桌面副本 SHA-256：`BC0A3DDA7608938CDC6105C3C9B36A505AF550EE89F5F91585E70713E9F92B51`，两者一致。
...
- 独立验证摘要：`tool/android/private-output/tv-test-20260807-111115-summary.json`，该目录为本机私有输出，不提交到仓库。

## 本轮自动化覆盖

- 本地媒体库内容区、路径输入框、搜索框和内层焦点范围按左键进入侧边导航栏；从侧栏按右键恢复内容焦点。
- TV 海报墙使用 260dp 最大卡片宽度、16dp 间距和紧凑安全边距，电影、动漫、电视剧与本地媒体库共用该布局策略。
...
| Windows Inno 安装 | 已安装 `2.1.143` | 注册表安装目录为 `D:\看影音`，主程序产品版本为 `2.1.143`；本轮 `2.1.144` 未执行安装 |
...
- 安装 2.1.144 TV APK，重测海报墙布局、所有子页面左键进入侧栏、弹窗与输入框返回、播放控制层返回、导入文件选择和目录顶部焦点提示。
