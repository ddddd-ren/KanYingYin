...
void main() {
  test('播放器面板和页面释放其持有的 Flutter 控制器', () {
    final panel = File('lib/pages/player/player_item_panel.dart')
        .readAsStringSync();
    final smallestPanel = File('lib/pages/player/smallest_player_item_panel.dart')
        .readAsStringSync();
    final videoPage = File('lib/pages/video/video_page.dart').readAsStringSync();
    final decoder = File('lib/pages/settings/decoder_settings.dart')
        .readAsStringSync();
  ...
  test('快捷键页面不在 build 中创建临时 FocusNode', () {
    final source = File('lib/pages/settings/keyboard_settings.dart')
        .readAsStringSync();
