...
void main() {
  ...
  test('Windows 二点一九六测试版构建版本面一致', () {
    ...
    expect(pubspec, contains('version: 2.1.196+20196'));
    expect(pubspec, contains('msix_version: 2.1.196.0'));
    expect(appVersion, contains("current = '2.1.196'"));
    expect(releaseNotes, contains('## 2.1.196+20196'));
    expect(releaseNotes, contains('Windows 测试版：2.1.196'));
    expect(updateDialogCopy, contains('应用版本：2.1.196'));
    expect(updateDialogCopy, contains('Windows EXE 安装器版本：2.1.196 测试版'));
    expect(versionHistory, contains("version: '2.1.196'"));
    expect(gradle, contains('windowsVersionName != "2.1.196"'));
    expect(gradle, contains('windowsVersionCode != 20196'));
    expect(gradle, contains('val androidVersionName = "1.0.7"'));
    expect(gradle, contains('val androidVersionCode = 10007'));
    expect(androidScript, contains("pubspecVersion.Name -ne '2.1.196'"));
    expect(androidScript, contains('pubspecVersion.Code -ne 20196'));
